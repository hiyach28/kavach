...
async def shield_check(
    ...
    logger.info(
        ...
        check_id, result.verdict, result.tier_resolved, latency_ms, user.id,
    )
...
async def _feed_flywheel(
    ...
    for h in entity_hashes[:20]:  # cap per check
        existing = (await db.execute(
            select(Entity).where(Entity.value_hash == h)
        )).scalar_one_or_none()
    ...
    if created_ids:
        ...
        shield_check = (await db.execute(
            select(ShieldCheck).where(
                ShieldCheck.entity_hashes.isnot(None),
                ShieldCheck.entity_hashes[0].as_string() == h
                if h else False,
            )
        )).scalar_one_or_none()
...
async def live_call_companion(
    ...
    try:
        while True:
            ...
            if triggered_stages:
                new_stage = "danger" if "payment" in triggered_stages or "threat" in triggered_stages else "caution"
                if new_stage != current_stage:
                    current_stage = new_stage
                    stage_label = triggered_stages[-1]
                    advice = _STAGE_ADVICE.get(stage_label, {}).get(lang, _STAGE_ADVICE[stage_label]["en"])
...
_IMPERSONATION_KW = [
    # English
    "police", "cbi", "narcotics", "customs", "fbi", "interpol", "court",
    "judge", "lawyer", "government officer", "commissioner", "senior",
...
_THREAT_KW = [
    # English
    "arrest", "warrant", "case filed", "notice", "summons", "legal action",
    "criminal case", "supreme court", "high court", "imprisonment", "jail",
    "non-bailable", "fir", "court case", "prosecution", "punishment",
    "drugs found", "parcel", "courier", "ndps", "money laundering",
    # Hindi
...
_PAYMENT_KW = [
    # English
    "upi", "payment", "transfer", "bank account", "otp", "send money",
    "digital arrest", "online payment", "scan code", "qr code",
    "net banking", "google pay", "phone pe", "paytm", "bank detail",
    "deposit", "fine", "penalty", "skype", "nodal officer",
    "account number", "ifsc", "verify your account",
    # Hindi
    "paisa", "rupay", "bhugtan", "khata", "accha", "transaction",
    "payment abhi karo", "account verify", "otp bhejo",
]

_STAGE_ADVICE: dict[str, dict[str, str]] = {
    "impersonation": {
        "en": "Government agencies never call to demand money. Hang up and verify through official channels.",
        "hi": "सरकारी एजेंसियां पैसे मांगने के लिए कभी फोन नहीं करतीं। फोन काटें और आधिकारिक माध्यमों से सत्यापित करें।",
        "ta": "அரசு நிறுவனங்கள் பணம் கேட்டு ஒருபோதும் அழைப்பதில்லை. அழைப்பை துண்டித்து அதிகாரப்பூர்வ வழிகளில் சரிபார்க்கவும்.",
        "te": "ప్రభుత్వ ఏజన్సీలు డబ్బు కోసం ఎప్పుడూ కాల్ చేయవు. ఫోన్ కట్ చేసి అధికారిక మార్గాల ద్వారా ధృవీకరించండి.",
        "bn": "সরকারি সংস্থাগুলো টাকা চেয়ে কখনো ফোন করে না। ফোন কেটে দিন এবং অফিসিয়াল চ্যানেলের মাধ্যমে যাচাই করুন।",
    },
    "threat": {
        "en": "This is a classic scare tactic. Real law enforcement sends written notice, not phone threats.",
        "hi": "यह एक क्लासिक डराने की रणनीति है। वास्तविक कानून प्रवर्तन फोन पर धमकी नहीं देता, बल्कि लिखित नोटिस भेजता है।",
        "ta": "இது ஒரு பொதுவான மிரட்டல் தந்திரம். உண்மையான சட்ட அமலாக்கம் தொலைபேசி மிரட்டல்களை அனுப்பாது, எழுத்துப்பூர்வ அறிவிப்பை அனுப்பும்.",
        "te": "ఇది ఒక సాధారణ భయపెట్టే వ్యూహం. నిజమైన చట్ట అమలు ఫోన్ బెదిరింపులు కాకుండా వ్రాతపూర్వక నోటీసును పంపుతుంది.",
        "bn": "এটি একটি ক্লাসিক ভীতি প্রদর্শনের কৌশল। প্রকৃত আইন প্রয়োগকারী সংস্থা ফোনে হুমকি না দিয়ে লিখিত নোটিশ পাঠায়।",
    },
    "isolation": {
        "en": "Scammers isolate victims to prevent them from seeking help. Tell someone you trust immediately.",
        "hi": "स्कैमर पीड़ितों को अलग-थलग करते हैं ताकि वे मदद न ले सकें। तुरंत किसी भरोसेमंद व्यक्ति को बताएं।",
        "ta": "மோசடி செய்பவர்கள் பாதிக்கப்பட்டவர்களை தனிமைப்படுத்தி உதவி கேட்க விடாமல் செய்கிறார்கள். உடனே நம்பகமான ஒருவரிடம் சொல்லுங்கள்.",
    ...
    "payment": {
        "en": "DO NOT SEND MONEY. This is a scam. No legitimate agency asks for payment over the phone.",
...
def _get_stage_alert(stage: str, lang: str) -> str:
    ...
    alerts = {
        "impersonation": {
            "en": "⚠️ The caller claims to be an authority figure. This is a common impersonation tactic.",
        ...
        "threat": {
            "en": "🚨 The caller is making threats. Real authorities never threaten you over the phone.",
        ...
        "payment": {
            "en": "🔴 DANGER: They're asking for money or payment details. THIS IS A SCAM. Hang up NOW.",
