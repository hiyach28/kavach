...
export default function DossierPanel() {
  ...
  if (!activeCase) {
    return (
      <div className="w-80 bg-bg-surface border-l border-border-hairline h-full flex flex-col items-center justify-center p-6 text-center select-none">
        <span className="font-mono text-xs text-text-secondary uppercase tracking-widest mb-2 font-bold animate-pulse">NO_ACTIVE_DOSSIER</span>
  ...
  const getSeverityBadge = (category) => {
    switch (category) {
      case 'critical':
        return <span className="px-1.5 py-0.5 rounded text-[13px] font-mono font-bold bg-sev-critical/20 text-sev-critical border border-sev-critical/30">CRITICAL</span>;
      case 'high':
        return <span className="px-1.5 py-0.5 rounded text-[13px] font-mono font-bold bg-sev-high/20 text-sev-high border border-sev-high/30">HIGH</span>;
      default:
        return <span className="px-1.5 py-0.5 rounded text-[13px] font-mono font-bold bg-mod-network/20 text-mod-network border border-mod-network/30">MEDIUM</span>;
    }
  };
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      case 'confirmed':
        return <span className="px-2 py-0.5 text-xs font-mono font-bold bg-sev-critical/20 text-sev-critical border border-sev-critical/30 uppercase rounded-full">CONFIRMED</span>;
      case 'false_positive':
        return <span className="px-2 py-0.5 text-xs font-mono font-bold bg-sev-verified/20 text-sev-verified border border-sev-verified/30 uppercase rounded-full">CLEARED</span>;
      case 'needs_manual_review':
        return <span className="px-2 py-0.5 text-xs font-mono font-bold bg-sev-high/20 text-sev-high border border-sev-high/30 uppercase rounded-full">PENDING_REVIEW</span>;
      default:
        return <span className="px-2 py-0.5 text-xs font-mono font-bold bg-accent-signal/20 text-accent-signal border border-accent-signal/30 uppercase rounded-full">CLASSIFIED</span>;
    }
  };
  ...
  return (
    <div className="w-96 bg-bg-surface border-l border-border-hairline h-full flex flex-col overflow-hidden select-none select-text">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-5">
        ...
        {/* Core Metadata */}
        <div className="bg-bg-base/30 p-3 rounded border border-border-hairline font-mono text-xs text-text-secondary space-y-1.5">
        ...
        <div className="flex items-center space-x-4 bg-bg-base/20 p-3 rounded border border-border-hairline">
          {activeCase.risk_score !== null ? (
            ...
            <div className="h-16 w-16 rounded-full border border-dashed border-sev-high bg-sev-high/10 text-sev-high flex flex-col items-center justify-center flex-shrink-0">
              <span className="font-mono text-xs text-center leading-tight font-bold">MANUAL<br/>REVIEW</span>
            </div>
          )}
          <div className="flex-1 min-w-0">
            <div className="font-mono text-[13px] text-text-secondary uppercase">LLM_VERDICT_CONFIDENCE: {(activeCase.confidence * 100).toFixed(0)}%</div>
            ...
            <div className="space-y-2">
              {activeCase.red_flags.map((flag, idx) => (
                <div key={idx} className="p-3 bg-bg-base/30 rounded border border-border-hairline space-y-2">
                  ...
                  <div className="font-mono text-xs text-text-primary bg-bg-base px-2 py-1 border border-border-hairline rounded leading-relaxed select-text italic">
                    "{flag.evidence}"
                  </div>
          ...
          {activeCase.infra && activeCase.infra.length > 0 ? (
            <div className="grid grid-cols-1 gap-1.5">
              {activeCase.infra.map((infraNode, idx) => (
                <div key={idx} className="flex items-center justify-between p-2 bg-bg-base/30 rounded border border-border-hairline font-mono text-xs">
                  <span className="text-text-primary">{infraNode}</span>
                  <span className="text-text-secondary text-xs tracking-tight bg-bg-base px-1.5 py-0.5 border border-border-hairline rounded">
                    SHA256_HASHED
                  </span>
                </div>
              ))}
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            ...
            {loadingLogs && <span className="font-mono text-xs text-text-secondary uppercase animate-pulse">FETCHING...</span>}
          </div>
          ...
          {auditLogs.length > 0 ? (
            ...
            <div className="p-2 text-center text-text-secondary text-xs font-mono border border-border-hairline rounded bg-bg-base/10">
              NO_LOG_RECORDS
            </div>
          )}
        </div>
      ...
      <div className="p-4 border-t border-border-hairline bg-bg-base/50 space-y-3">
        <span className="font-mono text-xs text-text-secondary uppercase tracking-wider block font-semibold text-center">INVESTIGATOR_DECISION_VERIFY</span>
